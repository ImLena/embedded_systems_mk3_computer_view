...
def process(frame):
    ...
    start_point2 = (int(height/2 - rect_size/2-200), int(width/2 - rect_size/2))
    end_point2 = (int(height/2 + rect_size/2-200), int(width/2 + rect_size/2))

    start_point3 = (int(height/2 - rect_size/2-100), int(width/2 - rect_size/2))
    end_point3 = (int(height/2 + rect_size/2-100), int(width/2 + rect_size/2))
   ...
   # color = (0, 0, 0)
    ...
    rect2 = cv2.rectangle(frame, start_point2, end_point2, color2, thickness)
    ...
    if pink_rate > 0.9:
        val4 = 1
        text4 = cv2.putText(rect4, ' OK (pink) ', org3, font, fontScale, color, thickness, cv2.LINE_AA)
    else:
    ...
    text = cv2.putText(rect, str(average) + " " + str(blue_rate), (10,50), font, fontScale, color, thickness, cv2.LINE_AA)

    #text2 = cv2.putText(rect, str(average) + " " + str(green_rate), (30, 60), font, fontScale, color, thickness,cv2.LINE_AA)
    frame = text
    return frame
